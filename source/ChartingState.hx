...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	private var iconLayer:FlxTypedGroup<HealthIcon> = new FlxTypedGroup<HealthIcon>();
	...
	override function create()
	{
		...
		snapText2 = new FlxText(60,50,0,"Press ALT for instakill notes\nX for damage notes\nZ for more dangerous notes\n" + 
			"G for gold notes\nF for a warning note\nV for white notes", 14);
		snapText2.scrollFactor.set();
		...
		iconLayer.add(leftIcon);
		iconLayer.add(rightIcon);
		add(iconLayer);
		...
		for (p in 0...pressArray.length)
		{
			...
			if (i && !delete)
			{
				...
				if (FlxG.keys.pressed.V)
					noteType = 6;
				addNote(new Note(Conductor.songPosition,p,null,false,false,true,noteType));
			}
		}
	...
	function updateHeads(?mode:Int):Void
	{
		switch(mode){
			case 1:
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			var note:Note = new Note(daStrumTime, daNoteInfo % 4,null,false,true, true, daType);
	...
	private function addNote(?n:Note):Void
	{
		...
		if (n != null)
			_song.notes[curSection].sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, n.specialType]);
		else
