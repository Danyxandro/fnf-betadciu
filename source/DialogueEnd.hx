...
class DialogueEnd extends FlxSpriteGroup
{
	...
	private var beginDialogue:Bool = false;
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		if(sys.FileSystem.exists("assets/data/" + PlayState.SONG.song.toLowerCase() + "/endingBGs.json")){
			var bgJson:Array<Dynamic> = cast Json.parse( sys.io.File.getContent( "assets/data/" + PlayState.SONG.song.toLowerCase() + "/endingBGs.json" ).trim() ).bg;
			trace(bgJson);
			for (ar in bgJson){
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'growth' | 'exploration' | 'control' | 'strange-comfort':
				hasDialog = true;
