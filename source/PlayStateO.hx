...
class PlayState extends MusicBeatState
{
	...
	public var healthValues:Map<String,Float> = [
		"shit" => -0.2,
		"bad" => -0.06,
		"good" => 0.04,
		"sick" => 0.1,
	...
	public var health2:Map<String,Float>;
									...
									else
									{
										switch(daNote.specialType){
											...
											if(daNote.isSustainNote && daNote.prevNote != null){
												health += healthValues["missLN"];
											}else{
												health += healthValues["miss"];
											}
											case 4:
											if(daNote.isSustainNote && daNote.prevNote != null){
												health += healthValues["type4missLN"];
											}else{
												health += healthValues["type4miss"];
											}
											case 5:
											if(daNote.isSustainNote && daNote.prevNote != null){
												health += healthValues["type5missLN"];
											}else{
												health += healthValues["type5miss"];
											}
											case 6:
											if(daNote.isSustainNote && daNote.prevNote != null){
												health += healthValues["type6missLN"];
											}else{
												health += healthValues["type6miss"];
											}
										}
										switch(daNote.specialType){
											case 0|4|5|6:
			...
			switch(daNote.specialType){
				...
				switch(daRating)
				{
					...
					case 'good':
						...
						goods++;
						if (health < 2)
							health += healthValues['good'];
						if (FlxG.save.data.accuracyMod == 0)
							totalNotesHit += 0.75;
					case 'sick':
						if (health < 2)
							health += healthValues['sick'];
						if (FlxG.save.data.accuracyMod == 0)
							totalNotesHit += 1;
						sicks++;
				}
				...
				switch(daRating)
				{
					...
					case 'good':
						...
						goods++;
						if (health < 2)
							health += healthValues['good6'];
						if (FlxG.save.data.accuracyMod == 0)
							totalNotesHit += 0.75;
					case 'sick':
						score = 500;
						if (health < 2)
							health += healthValues['sick6'];
						if (FlxG.save.data.accuracyMod == 0)
							totalNotesHit += 1;
						sicks++;
				}
							...
							for (shit in 0...pressArray.length)
								{ // if a direction is hit that shouldn't be
									if (pressArray[shit] && !directionList.contains(shit)){
										health += healthValues["missPressed"];
										noteMiss(shit, null);
									}
								}
						}
		...
		switch (difficulty)
		{
			...
			case 2:
				...
				healthValues = [
					...
					"type6miss" => -0.3,
					"type6missLN" => -0.1
				]; //Kade Engine 1.8 values
			case -1:
				health2 = [
					for (key in healthValues.keys())
						key => healthValues[key]
				];
				healthValues.set("shit",0);
				healthValues.set("bad",0);
				healthValues.set("miss",0);
				healthValues.set("missLN",0);
				healthValues.set("type4miss",0);
				healthValues.set("type5miss",0);
				healthValues.set("type4missLN",0);
				healthValues.set("type5missLN",0);
				healthValues.set("shit6",0);
				healthValues.set("bad6",0);
				healthValues.set("type6miss",0);
				healthValues.set("type6missLN",0);
				SONG.validScore = false;
			case -2:
				for (key in health2.keys())
					healthValues[key] = health2[key];
		}
